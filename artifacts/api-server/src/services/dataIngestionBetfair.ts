...
export async function runBetfairIngestion(): Promise<void> {
  ...
  const eventIds = upcomingMatches
    ...
    .filter(
      (id): id is string => id !== null && !id.startsWith("fd_"),
    );
  ...
  const marketCatalogueMap = new Map(
    marketItems.map((m) => [m.marketId, m]),
  );
  ...
  for (const marketId of marketIds) {
    ...
    const marketType = catalogue.description?.marketType ?? "UNKNOWN";
  ...
  logger.info({ durationMs }, "Betfair ingestion complete");
}
